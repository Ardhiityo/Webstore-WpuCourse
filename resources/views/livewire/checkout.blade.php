                   ...
                   <div class="grid grid-cols-2 gap-3 mt-2">
                       ...
                       <div>
                           <input type="text" wire:model='data.email'
                               class="py-1.5 sm:py-2 px-3 pe-11 @error('data.email') border-red-600
                               @enderror block w-full border-gray-200 shadow-2xs sm:text-sm rounded-lg focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600"
                               placeholder="Email">
                           @error('data.email')
                               <p class="mt-2 text-xs text-red-600" id="hs-validation-name-error-helper">
                                   {{ $message }}
                       ...
                       <div>
                           <input type="text" wire:model='data.phone'
                               class="@error('data.phone') border-red-600
                           ...
                           <div x-data="{ open: false }" class="relative w-full">
                               <div class="relative">
                                   ...
                                   <div wire:loading wire:target='region_selector.keyword'
                                       class="animate-spin inline-block size-4 border-3 border-current border-t-transparent text-blue-500 rounded-full dark:text-blue-500 absolute top-3 right-3"
                                   ...
                                   <ul class="absolute z-10 w-full mt-1 overflow-y-auto bg-white border border-gray-200 rounded-b-lg max-h-60"
                                       ...
                                       @foreach ($this->regions as $region)
                                           <li class="p-2 cursor-pointer hover:bg-gray-100">
                                               <label for="region-code-{{ $region->code }}"
                                                   class="w-full inline-block cursor-pointer"> {{ $region->label }}
                                                   <input type="radio" class="sr-only" name="region_code"
                                                       value="{{ $region->code }}"
                                                       wire:model.live='region_selector.region_selected'
                                                       id="region-code-{{ $region->code }}">
                                               </label>
                                           </li>
                                       @endforeach
                                   </ul>
               ...
               <div class="mt-2 space-y-3">
                   <div class="flex justify-center">
                       <div class="animate-spin size-6 border-3 border-current border-t-transparent text-blue-500 rounded-full dark:text-blue-500"
                           ...
                           wire:target='region_selector.region_selected'>
                       </div>
                   </div>
                   <div class="grid space-y-2">
                       @forelse ($this->shipping_methods as $group => $shipping_methods)
                               ...
                               <label for="shipping_method_{{ $shipping_method->hash }}"
                                   ...
                                   <div class="flex items-center justify-start gap-2">
                                       ...
                                       @if ($shipping_method->logo_url)
                                           <img src="{{ $shipping_method->url }}" class="h-5" />
                                       @endif
                                       <span class="text-sm text-gray-500 ms-3 dark:text-neutral-400">
                                           <span class="text-gray-500"> {{ $shipping_method->label }}</span>
                                       </span>
                                   </div>
                   ...
                   <div class="grid space-y-2">
                       @foreach ($this->payment_methods as $key => $payment_method)
                           <label for="payment_method_{{ $payment_method->hash }}"
                               ...
                               <span
                                   class="text-sm text-gray-500 ms-3 dark:text-neutral-400">{{ $payment_method->label }}
                               </span>
                           </label>
                       @endforeach
