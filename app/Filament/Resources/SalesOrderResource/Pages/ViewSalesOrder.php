        ...
        return [
            Action::make('process')
                ...
                ->visible(fn() => in_array(get_class($this->record->status), [
                    ...
                    $options = collect($transitions)->mapWithKeys(
                        fn($class) => [$class => (new $class($this->record))->label()]
                    )->toArray();
            ...
            Action::make('Input Resi Pengiriman')
                ...
                ->visible(function () {
                    ...
                    app(SalesOrderService::class)->updateShippingReceipt(SalesOrderData::fromModel($this->record), data_get($data, 'shipping_receipt_number'));
                })
        ];
    }
}
